...
CONTRACT tictactoe : public contract {
    ...
    ACTION move(name opponent, name host, name by, uint16_t row, uint16_t col) {
		...
		_game.modify(*itr, same_payer, [&]( auto& game ) {
			...
			if (game.winner!=name()) {
				payback(game.winner, game.hoststake+game.opponentstake, "You got the prize.");
			}
			else if (game.is_draw()) {
				payback(game.host, game.hoststake, "Stake refund.");
				payback(game.opponent, game.opponentstake, "Stake refund.");
			}	 
		});
	}
